...
let runtimeStatusBar;
let typingStatusBar;
let speedStatusBar;
let wordCountStatusBar;
...
function activate(context) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('novelWritingAssistant.openConfig', openConfigFile), vscode.commands.registerCommand('novelWritingAssistant.highlightSelection', handleHighlightSelection), vscode.commands.registerCommand('novelWritingAssistant.clearHighlights', clearHighlightRules), vscode.window.registerWebviewViewProvider('novelWritingAssistantView', viewProvider), vscode.window.onDidChangeActiveTextEditor(() => updateAll()), vscode.workspace.onDidChangeTextDocument(event => {
        recordDocumentChange(event);
        updateAll();
    }), vscode.workspace.onDidOpenTextDocument(() => updateAll()), vscode.workspace.onDidCloseTextDocument(() => updateAll()));
}
function deactivate() {
    if (timer) {
        clearInterval(timer);
...
async function saveConfig(extensionPath) {
...
function recordDocumentChange(event) {
...
function getCurrentTypingMs(now) {
    if (typingActive) {
...
function findAllRanges(document, text) {
    if (!text) {
        return [];
    }
...
class NovelWritingAssistantViewProvider {
            ...
            switch (message.type) {
                ...
                case 'save-settings': {
                    ...
                    config.highlightItems = rawRules
                        .map((rule) => ({ text: String(rule.text ?? '').trim(), color: String(rule.color ?? '#ffeb3b') }))
